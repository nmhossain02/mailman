...
func (s *DB) SaveCommandCorrection(ctx context.Context, v core.CommandCorrection) error {
...
func (s *DB) SaveEvalRun(ctx context.Context, v inference.EvalRunConfig, status string, now time.Time) error {
	b, err := marshal(v)
	if err != nil {
		return err
	}
