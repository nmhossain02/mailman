...
func (s *DB) PutTrace(ctx context.Context, trace inference.InferenceTrace) error {
...
func (s *DB) Trace(ctx context.Context, id string) (inference.InferenceTrace, error) {
	var t inference.InferenceTrace
	var b []byte
...
func (s *DB) BeginOperation(ctx context.Context, executionKey, requestHash string, now time.Time) (journal.Entry, bool, error) {
	var e journal.Entry
	var response []byte
	var updated string
	...
	e = journal.Entry{ExecutionKey: executionKey, RequestHash: requestHash, State: "pending", Response: json.RawMessage(`null`), UpdatedAt: now.UTC()}
