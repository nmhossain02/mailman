...
func (r *Router) Route(ctx context.Context, request RouteRequest) (RouteResult, error) {
	...
	case "external_only":
		...
		return RouteResult{Selected: result, Traces: []core.InferenceTrace{trace}}, err
	case "local_only", "fallback", "probe":
		local, localTrace, localErr := call(ctx, r.Local, request, "local", "production", "local_first", true)
		out := RouteResult{Selected: local, Traces: []core.InferenceTrace{localTrace}}
...
func (r *Router) callExternal(ctx context.Context, request RouteRequest, role, reason string) (TaskResult, core.InferenceTrace, error) {
...
func call(ctx context.Context, backend Backend, request RouteRequest, class, role, reason string, selected bool) (TaskResult, core.InferenceTrace, error) {
	started := time.Now()
	if backend == nil {
