...
func TestInferExactRequestAndResult(t *testing.T) {
	...
	result, err := b.Infer(context.Background(), inference.Request{Model: "qwen", Instructions: "safe", InputJSON: json.RawMessage(`{"x":1}`), OutputSchema: json.RawMessage(`{"type":"object"}`), MaxOutputTokens: 20})
	if err != nil {
		t.Fatal(err)
	}
...
func TestInferTruncation(t *testing.T) {
	...
	defer server.Close()
	_, err := (&Backend{BaseURL: server.URL, Client: server.Client()}).Infer(context.Background(), inference.Request{OutputSchema: json.RawMessage(`{}`)})
	if err == nil {
