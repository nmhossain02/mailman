...
func TestJSONLRoundTripAndHashValidation(t *testing.T) {
	records := []DatasetRecord{{Case: core.EvalCase{ID: "b", Dataset: "mail", TaskName: "message_kind", TaskVersion: "1", InputJSON: json.RawMessage(`{"subject":"B"}`)}}, {Case: core.EvalCase{ID: "a", Dataset: "mail", TaskName: "message_kind", TaskVersion: "1", InputJSON: json.RawMessage(`{"subject":"A"}`)}, InputReference: "fixture:a", ExpectedJSON: json.RawMessage(`{"kind":"work"}`)}}
...
func TestProbeGroupsPairedOutputsAndKeepsLocal(t *testing.T) {
	records := []DatasetRecord{{Case: core.EvalCase{ID: "c1", Dataset: "d", TaskName: "message_kind", TaskVersion: "1", InputJSON: json.RawMessage(`{}`)}}}
	config := Snapshot("run", "d", RouteProbe)
	config.MaxExternalCalls = 1
	result, err := Run(context.Background(), config, records, func(_ context.Context, _ core.EvalCase, backend string) Observation {
...
func TestOracleSelectsHigherScoreAndBreaksTieLocal(t *testing.T) {
	records := []DatasetRecord{
		{Case: core.EvalCase{ID: "better", Dataset: "d", TaskName: "message_kind", TaskVersion: "1", InputJSON: json.RawMessage(`{}`)}, ExpectedJSON: json.RawMessage(`{"kind":"work","abstain":false}`)},
		{Case: core.EvalCase{ID: "tie", Dataset: "d", TaskName: "message_kind", TaskVersion: "1", InputJSON: json.RawMessage(`{}`)}, ExpectedJSON: json.RawMessage(`{"kind":"work","abstain":false}`)},
	}
	config := Snapshot("run", "d", RouteOracle)
	config.MaxExternalCalls = 2
	result, err := Run(context.Background(), config, records, func(_ context.Context, c core.EvalCase, backend string) Observation {
		kind := "work"
...
func TestExternalModesHonorPositiveCap(t *testing.T) {
	record := DatasetRecord{Case: core.EvalCase{ID: "c", Dataset: "d", TaskName: "message_kind", TaskVersion: "1", InputJSON: json.RawMessage(`{}`)}}
	_, err := Run(context.Background(), Snapshot("run", "d", RouteExternalOnly), []DatasetRecord{record}, func(context.Context, core.EvalCase, string) Observation { return Observation{} })
	if err == nil {
