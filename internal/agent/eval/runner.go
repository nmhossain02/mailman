...
type ExecuteFunc func(context.Context, agent.EvalCase, string) Observation
...
type RunResult struct {
	Config  agent.EvalRunConfig `json:"config"`
	Cases   []CaseResult        `json:"cases"`
	Metrics Metrics             `json:"metrics"`
}

func Snapshot(id, dataset, mode string) agent.EvalRunConfig {
	return agent.EvalRunConfig{ID: id, Dataset: dataset, RouteMode: mode, Concurrency: 1, CacheEnabled: false}
}

func FiveSnapshots(dataset string) []agent.EvalRunConfig {
	modes := []string{RouteLocalOnly, RouteExternalOnly, RouteFallback, RouteProbe, RouteOracle}
	out := make([]agent.EvalRunConfig, 0, len(modes))
...
func Run(ctx context.Context, config agent.EvalRunConfig, records []DatasetRecord, execute ExecuteFunc) (RunResult, error) {
	if execute == nil {
