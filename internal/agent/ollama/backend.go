...
func (b *Backend) Health(ctx context.Context) agent.Health {
	req, err := http.NewRequestWithContext(ctx, http.MethodGet, strings.TrimRight(b.BaseURL, "/")+"/api/tags", nil)
	if err != nil {
		return agent.Health{SafeMessage: "invalid Ollama URL"}
	}
	client := b.Client
	if client == nil {
	...
	if err != nil {
		return agent.Health{SafeMessage: "Ollama unavailable"}
	}
	defer resp.Body.Close()
	if resp.StatusCode/100 != 2 {
		return agent.Health{SafeMessage: fmt.Sprintf("Ollama returned HTTP %d", resp.StatusCode)}
	}
	var body struct {
		Models []struct{ Name, Digest string } `json:"models"`
	}
	if json.NewDecoder(resp.Body).Decode(&body) != nil {
		return agent.Health{SafeMessage: "invalid Ollama health response"}
	}
	revision := ""
	...
	return agent.Health{Ready: true, ModelRevision: revision}
}
...
func (b *Backend) Infer(ctx context.Context, request agent.Request) (agent.ProviderResult, error) {
	...
	if err := json.Unmarshal(data, &response); err != nil {
		return agent.ProviderResult{}, invalid("invalid Ollama response")
	}
	if !response.Done || response.DoneReason == "length" {
		return agent.ProviderResult{}, invalid("Ollama output was truncated")
	}
	if !json.Valid([]byte(response.Message.Content)) {
		return agent.ProviderResult{}, invalid("Ollama returned malformed JSON")
	}
