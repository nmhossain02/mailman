...
func (b *Backend) Health(context.Context) agent.Health {
	if strings.TrimSpace(b.APIKey) == "" {
		return agent.Health{SafeMessage: "OpenAI API key is not configured"}
	}
	return agent.Health{Ready: true}
}

func (b *Backend) Infer(ctx context.Context, request agent.Request) (agent.ProviderResult, error) {
	var schema any
	if err := json.Unmarshal(request.OutputSchema, &schema); err != nil {
		return agent.ProviderResult{}, &agent.InferenceError{Kind: "invalid_request", SafeMessage: "invalid output schema"}
	}
	...
	started := time.Now()
	_, data, err := agent.DoJSON(ctx, b.Client, func() (*http.Request, error) {
	...
	if err := json.Unmarshal(data, &response); err != nil {
		return agent.ProviderResult{}, invalid("invalid OpenAI response")
	}
	if response.Status != "completed" {
		...
		return agent.ProviderResult{}, &agent.InferenceError{Kind: kind, SafeMessage: "OpenAI response was " + response.Status}
	}
	...
	for _, item := range response.Output {
		...
		for _, content := range item.Content {
			switch content.Type {
			case "refusal":
				return agent.ProviderResult{}, &agent.InferenceError{Kind: "refused", SafeMessage: "OpenAI refused the request"}
			case "output_text":
				if outputText != "" {
					return agent.ProviderResult{}, invalid("OpenAI returned multiple output texts")
				}
...
func invalid(message string) error {
	return &agent.InferenceError{Kind: "invalid_output", SafeMessage: message}
}
