...
func TestInferExactRequestAndResult(t *testing.T) {
	...
	defer server.Close()
	result, err := (&Backend{BaseURL: server.URL, APIKey: "secret", Client: server.Client()}).Infer(context.Background(), agent.Request{TaskName: "kind", Model: "gpt-test", InputJSON: json.RawMessage(`{}`), OutputSchema: json.RawMessage(`{"type":"object"}`), MaxOutputTokens: 50})
	if err != nil {
		t.Fatal(err)
	}
...
func TestTerminalStates(t *testing.T) {
	...
	for name, response := range cases {
		t.Run(name, func(t *testing.T) {
			...
			defer server.Close()
			_, err := (&Backend{BaseURL: server.URL, APIKey: "x", Client: server.Client()}).Infer(context.Background(), agent.Request{OutputSchema: json.RawMessage(`{}`)})
			if err == nil {
