...
type InferenceStore interface {
	...
	PutTrace(context.Context, InferenceTrace) error
}
...
func (r PrimitiveRunner) Run(ctx context.Context, taskName, targetID string, input json.RawMessage, route core.RoutePolicy) (TaskResult, error) {
	if r.Router == nil || r.Store == nil {
		return TaskResult{}, fmt.Errorf("primitive runner is not configured")
	}
	if r.Router.Local == nil && route.Mode != "external_only" {
		return TaskResult{}, fmt.Errorf("local inference backend is not configured")
	}
	task, err := BuiltinTask(taskName, r.LocalModel)
	if err != nil {
		return TaskResult{}, err
	}
	key, err := CacheKey(r.Router.Local.ID(), r.LocalRevision, task, input)
	if err != nil {
		return TaskResult{}, err
	}
